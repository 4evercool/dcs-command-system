...
### D6: Independence/regenerability funding — three relocations (incident `independence-fail-closed-and-model-floor`, 2026-08-03)

Stating criterion 5's independence/regenerability rule (principle 7's HALT extension), the `checked` field's regenerable-command requirement (schemas.md #5, Safety-officer verdict), and a new `approved_models` Delegation field (schemas.md #7, Delegation bounds) needed more hot-path bytes than either addition's own section could fund inside the fixed budget (`tests/test_doctrine_integrity.py`'s `HOT_PATH_BUDGET_KB`). Three passages were compressed to fund it, all in `doctrine.md`; schemas.md carries no compression this incident. All three originals are preserved verbatim below, per this incident's own tasking (never delete hot-path prose without routing it here).

**(1) The original "Relationship to project-specific protocols" second paragraph**, compressed in place under its own unchanged heading:
...
Removed: the MCP-server/query-interface/script tool-type examples, the "`/dcs-init` surfaces this at onboarding" sentence, and the environment-dependent tail (a tool absent for one installer may exist for another). Preserved: all three operative guarantees — a protocol an agent cannot execute is a charter defect, not an agent failure (v0.5.6); the role must be granted the tool in `agents/dcs-*.md`; the correct response is to widen the charter and flag the gap, never let a substitution stand unremarked.

**(2) The original "The lifecycle" closing paragraph**, compressed in place under its own unchanged heading:
...
Removed: the explanatory aside that running several periods is the exception not the aim, the parenthetical naming the follow-up's 201 evidence as this incident's own AAR, and the closing instruction to say which in the log. Preserved: the definition of one operational period, the `(v0.5.12)` tag, and the operative default itself — close, merge, ship, and register the remainder as a follow-up incident, keeping the incident open only when the delivered part genuinely cannot ship alone.

**(3) The original worktree-audit step 5**, compressed in place under its own unchanged heading ("Parallel operation"):
...
Removed: the "cryptic error (cwd self-conflict)" explanatory gloss, the "platform-specific" qualifier on the diagnostic commands before "(appendix)" (D4 already relocated those commands themselves, so only the qualifier is redundant here), and the restated "into the worktree"/"manual removal is needed" phrasing. Preserved: every operative step — check cwd and `cd` to `<esg_root>` first; if removal still fails, identify lock holders per the appendix and write `.dcs/CLOSED` as the terminal fallback (principle 11's zombie rule); tell the Owner; log the diagnostic output.

**Design note, `approved_models` (Safety Officer advisory, this incident):** the model floor reads the session's own self-reported identity — the same signal doctrine's "Transfer of command" already relies on to decide whether to spawn `dcs-commander`. It is an honesty-dependent control, adopted as such (`vault/Decisions/non-anthropic-hardening.md` explicitly rejects in-workflow model-quality heuristics in favor of one Owner-controlled list): it demotes an honest unlisted operator to supervised mode and does nothing against a dishonest self-report. Shipped text states the rule, never a stronger detection guarantee than that.
