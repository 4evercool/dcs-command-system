...
| `IAP-APPROVED` | IC, on Owner approval | `/dcs-plan`, after AskUserQuestion approval, or step 6c's amendment path | sha256 of `IAP.md`'s raw bytes at the moment of approval, plus approver/timestamp metadata. First line is always the bare hex hash — the gate hook and readers accept it if it matches any of three digests computed from the current `IAP.md` (raw bytes, LF-normalised, or CRLF-normalised derived from the LF form), so a line-ending-only drift after approval does not void the marker; the hash the IC stamps is always the raw-bytes digest, which is a member of that set by construction |
