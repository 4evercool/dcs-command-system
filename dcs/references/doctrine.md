...
13. **Escalation triggers are mandatory** (v0.2) — the IC MUST file a 209 and pause the incident when ANY of: (a) scope grows beyond the IAP's blast radius; (b) Safety halts twice on the same objective; (c) the incident enters period N+1, N = `esg.max_periods_before_review` (default 3); (d) a Delegation bound would be crossed. Continue / pivot / demobilize is always the Owner's decision, never the IC's alone.
