...
**Model availability:** "Fable" = the strongest tier available. If `dcs-commander` with `model: fable` fails, re-spawn with the strongest that works (`opus`, then `sonnet`) and log the actual seat (`command: ... (IC=dcs-commander, opus — fable unavailable at this attempt)`). **Availability is per-spawn and MUST be re-tested at every command point (v0.6.1).** A tier that failed earlier in this session or incident tells you nothing about now: quota limits are time-windowed and restore, and any incident lasting hours routinely outlives the window that produced the failure. "Fable is exhausted" is a derived fact with a lifetime (principle 15) — true when measured, false when read, and once written into an append-only log it gets cited as a standing condition by every later attempt. **Never cache the fallback, and never let one failed spawn demote the seat for the rest of the incident**; try the preferred tier first every single time, and if it fails again the cost is one wasted spawn, which is trivially cheaper than running the remaining command points at a lower tier than the Owner is paying for. NEVER acceptable: the fallback drifting to "the Dispatcher decides itself" — the separate spawn preserves a fresh context, defined inputs, and a logged decision, even same-tier.

### A command point is never a silent wait

**(v0.5.10)** Two rules, both about making the pause legible instead of ambiguous:
...
- **An empty or errored return is a FAILED spawn, not a slow one.** A spawn that ends with no decision block — quota exhausted, API error, early termination — is dead; the decision will never arrive. Re-spawn immediately on the next tier and log **both** attempts (the failure and the seat that answered). Never wait indefinitely on a corpse, never resume it (principle 9b), and never let a dead spawn become the reason the Dispatcher decides alone. **Liveness is measured by the decision, never by a proxy.** A spawn is dead only if it ended without a decision block — not because its transcript file looks empty, not because no files changed, not because it has been quiet. Transcript size and file mtimes are harness artifacts (buffers flush late; a command-point agent writes nothing by design), so reading death into them produces a confident wrong diagnosis and, worse, invites logging a failure that never occurred — the exact derived-fact error principle 15 forbids, now in an append-only log. Field lesson 2026-07-24: Fable quota exhaustion genuinely killed one command-point spawn, which was caught and re-spawned on `opus`; a later spawn was then misdiagnosed as dead from a zero-byte transcript when it had in fact returned a complete `reject` verdict, and acting on that misdiagnosis would have written a fabricated failed-attempt entry into the 214. **Ask the session what the agent returned; never infer it from the filesystem.**
...
6. **Terrain partitioning** — the IAP declares disjoint file territories per specialist; overlap ⇒ sequential stages or worktree isolation. The IC **rejects** any IAP lacking a partition and re-spawns the chief. **(v0.3)** Same rule portfolio-wide: concurrent incidents hold disjoint territories too (`REGISTER.md`'s `territory` column) — keeps their `git merge --no-ff` into main trivially clean. **(v0.6.2) And territory never leaves its own project: one session, one project.** Every DCS artifact is resolved relative to the project root that holds the `.dcs/` — config, `ACTIVE`, incidents, register, delegation, worktrees — so a session rooted in repo A opens A's incidents against A's portfolio, and repo B's work is B's own incident from a session rooted in B. This matters most where it is least visible: the gate cannot judge a tree it has no `.dcs/` for, so it *allows* every target outside the project. A cross-project territory is therefore silently ungated — which is why `plan.md` lint check 8 refuses it at plan time rather than leaving it to be discovered.
...
13. **Escalation triggers are mandatory** (v0.2) — the IC MUST file a 209 and pause the incident when ANY of: (a) scope grows beyond the IAP's blast radius; (b) Safety halts twice on the same objective — **and from that second halt on, the 209 MUST carry a convergence read (v0.5.9): are the new refutations the SAME class as the last halt's (whack-a-mole against an unenumerated population — a defect of form, not of site) or a different class (complex but converging)? Same-class recurrence means each further pass buys one instance at the cost of a full execute+verify cycle, so the options must lead with raising the fix's altitude — a guard that makes the class unrepresentable — never with an unqualified "continue"**; (c) the incident enters **attempt** N+1, N = `esg.max_periods_before_review` (default 3) — **an attempt is any stamped-and-executed IAP, so a re-planned revision of the same period counts (v0.5.12).** Counting periods alone is a loophole the field walked straight through: an incident ran **four revisions inside period 1** and logged, correctly under the old wording, *"(c) does not fire — revisions are not counted as periods"*, so a 31-hour thrash never tripped the cap that exists to catch exactly that; (d) a Delegation bound would be crossed; **(f)** the IAP is rejected a third time in one period (v0.5.1 — three rejects means the objectives, the chief's information diet, or the incident's size is wrong, not that the plan needs one more pass; offer continue / re-scope the 202 / decompose into separate incidents). Continue / pivot / demobilize is always the Owner's decision, never the IC's alone.
...
One operational period = one pass through the P-loop. **(v0.5.12) An incident may run several before closing, but that is the exception, not the shape to aim for:** a Safety-passed period holds *proven* work, and holding it open keeps that work unmerged and unshipped — fixing nothing — until the rest of the scope catches up. Default at a pass is **close, merge, ship, and register the remainder** as a follow-up incident (its 201 evidence is this incident's AAR). Keep it open only when the delivered part genuinely cannot ship alone, and say which in the log.
