...
- **An empty or errored return is a FAILED spawn, not a slow one.** A spawn that ends with no decision block — quota exhausted, API error, early termination — is dead; the decision will never arrive. Re-spawn immediately on the next tier and log both attempts. Never wait on a corpse, never resume it (principle 9b), and never let a dead spawn become the reason the Dispatcher decides alone. **Liveness is measured by the decision — ask the session what the agent returned, never infer it from the filesystem.**
...
7. **Independent safety authority** — verification is never done by the section that produced the work. A verdict whose independent-agent origin cannot be established, or whose `checked` entries do not reproduce, is a HALT, never a silent pass.
8. **Deviation doctrine** — specialists report deviations, never improvise a fix for a plan they've decided is wrong; a deviation sends the IC back to Tactics. Editing the IAP after approval invalidates it automatically (`IAP-APPROVED`'s hash no longer matches). **(v0.6.11)** Not every deviation costs a full Tactics pass — `plan.md`'s bounded amendment path is the proportionate route within its stated limits; outside them, the full path still applies.
...
13. **Escalation triggers are mandatory** (v0.2) — the IC MUST file a 209 and pause the incident when ANY of: (a) scope grows beyond the IAP's blast radius; (b) Safety halts twice on the same objective — **and from that second halt on, the 209 MUST carry a convergence read (v0.5.9): are the new refutations the SAME class as the last halt's (whack-a-mole against an unenumerated population — a defect of form, not of site) or a different class (complex but converging)? Same-class recurrence means the options must lead with raising the fix's altitude — a guard that makes the class unrepresentable — never with an unqualified "continue".** The per-attempt ceiling (`SAFETY-HALT:` counting via `dcs_gate.py`'s `halt_cycles()`, reset by `IAP-APPROVED:` and `SAFETY-PASS:`, grammar: "An entry begins at column zero with a mandatory bracketed timestamp; any other line is a continuation, never a sentinel, and quoting a whole prior entry inside a body requires indenting it off column zero.") lives in `dcs_gate.py`; the full mechanics text is in the appendix — a fourth sentinel-shaped token, `RECORD-CORRECTION:`, shares that column-zero boundary but is parsed only by `record_integrity.py`, never by `sentinel_of()` or counted here; (c) the incident enters **attempt** N+1, N = `esg.max_periods_before_review` (default 3) — **an attempt is any stamped-and-executed IAP, so a re-planned revision of the same period counts (v0.5.12).** (d) a Delegation bound would be crossed; **(f)** the IAP is rejected a third time in one period (v0.5.1 — offer continue / re-scope the 202 / decompose into separate incidents). Continue / pivot / demobilize is always the Owner's decision, never the IC's alone.
...
15. **No derived facts in durable artifacts** (v0.5.2) — an artifact that outlives the moment (202, 204, IAP, SAFETY, AAR, ADR, code comment, project memory) may not state a **derived** fact — a count, a hash, a version, an enumeration, an "only these two are X" census — without the command that regenerates it beside it. **Write the derivation, not the result.** Where regeneration is genuinely impossible, the minimum is an explicit `as of <ref> — it moves` annotation; an unqualified number is never acceptable. Default: an artifact-hygiene finding is an advisory, not a halt, unless it clears one of the bars `agents/dcs-safety-officer.md` step 6 names. Enforced there, not by discipline. **(v0.5.4) Tests are the sharpest case, and invert the failure.** A regression test must pin **immutable** evidence (a fixture, a frozen blob, a commit SHA), never a **moving ref** (branch name, `HEAD`, "the integration tip"). Where a test must observe live state, assert the **invariant**, never the **instance**. **(v0.6.11)** The same rule governs transfer between seats: a fact a prior seat already established moves into a later artifact by file reference or regenerating command, never by a later seat retyping it from memory — the Dispatcher copies bytes or cites paths, it never retypes a value.
16. **Close-time record integrity is mechanical, not behavioral** — a close runs a mechanical record-integrity check over the incident's own artifacts (`dcs/tools/record_integrity.py`) before the merge, unconditionally, never behind a project's own opt-in. A red result is fail-closed: it blocks the merge, is escalation trigger (a), and is never silently resolved.
...
One operational period = one pass through the P-loop. **(v0.5.12)** Default at a pass is **close, merge, ship, and register the remainder** as a follow-up incident; keep it open only when the delivered part genuinely cannot ship alone.

## v0.1 constraints (deliberate, not oversights)

- **One incident per worktree** (v0.3) — `.dcs/ACTIVE` is per-worktree (git-ignored, never merges); one seat, one `ACTIVE` file, one incident, scoped to whichever tree the session is rooted in. Concurrent-incident territory partition is governed by the register (principle 6; "Parallel operation" below).
...
**A protocol an agent cannot execute is a charter defect, not an agent failure (v0.5.6)** — every DCS role expected to honor a project protocol naming a tool must be *granted* it in `agents/dcs-*.md`: widen the charter, never let the substitution stand unremarked; flagging the gap is right, the defect is upstream.
...
5. **Removal refused → diagnose before escalating (v0.7.0, incident worktree-removal-self-conflict).** Check whether the shell's cwd is inside the worktree path first (`cd` to `<esg_root>` if so); if removal still fails, identify lock holders (appendix) and write `.dcs/CLOSED` (terminal fallback — principle 11's zombie rule), tell the Owner, and log the diagnostic output.
