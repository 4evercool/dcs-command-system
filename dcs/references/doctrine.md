...
5. **Removal refused → diagnose before escalating (v0.7.0, incident worktree-removal-self-conflict).** Before `git worktree remove`, check whether the shell's cwd is inside the worktree path — if so, cd to `<esg_root>` first, as removing the directory you are standing in fails with a cryptic error (cwd self-conflict). If removal still fails: identify lock holders with a platform-specific diagnostic — POSIX: `lsof +D <path>` or `fuser -v <path>`; Windows: `powershell "Get-Process | Where-Object { $_.Path -like '*<path>*' }"` (or Sysinternals `handle <path>` if installed). Write `.dcs/CLOSED` into the worktree (terminal fallback — `dcs_gate.py`'s zombie rule (principle 11) denies guarded edits while it exists), tell the Owner manual removal is needed, and include the diagnostic output in 214-LOG.md.
