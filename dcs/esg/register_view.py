...
Regenerate with:  python .dcs/register_view.py
...
Lives at <project>/.dcs/register_view.py, NOT inside .dcs/esg/ -- unlike
REGISTER.md, this is a static tool, not per-worktree portfolio data, so
it must survive a plain `git clone` (dcs_gate.py gets the same tracked
treatment at .claude/hooks/). Shipped from dcs/esg/register_view.py
(package source, installed to ~/.claude/dcs/esg/ by install.ps1);
/dcs-init copies it into each onboarded project.
"""
import argparse
import html
import re
...
def find_project_root(start):
    """Walk up from `start` (this script's own directory) looking for a
    .dcs/ directory -- the same resolution dcs_gate.py and
    register_view_regen.py use, so this script finds the same root no
    matter how deep under .dcs/ it happens to be installed. A hardcoded
    parents[N] distance already broke once when this file moved from
    vault/_scripts/ to dcs/esg/, and would have broken again moving here
    -- this walk makes any future move free.
    No .dcs/ found above `start` -> return `start` unchanged; the
    resulting DEFAULT_SOURCE simply won't exist, which main()'s existing
    "source file not found" check already reports without crashing."""
...
REGEN_CMD = "python .dcs/register_view.py"
