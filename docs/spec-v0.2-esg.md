...
1. **Prepare (Chief of Staff hat).** Read `.dcs/esg/*` (create from templates on first run, telling the Owner it's the founding session). Sweep intake sources: `REGISTER.md` queued rows, plus project-specific sources if the project's CLAUDE.md documents them (bread_bot: `audit_results` rows flagged `needs_fix` via the Remote Diagnostics read-only pattern; open items in `vault/dashboard/open-bugs.md` and `vault/Память/tech-debt.md`). Spawn 1–2 `dcs-situation-analyst` (read-only) for the sweep if it needs file/DB digging — never do prod writes.
