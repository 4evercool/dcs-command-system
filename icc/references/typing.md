# ICC Incident Typing Guide

Decided once, at the stem (`/icc-new`), before any planning happens.
...
Type-3 (or worse, Type-5) informality risks exactly the failure ICC exists
...
**Activation:** IC + 1 `icc-ops-specialist`. No chiefs, no Safety Officer,
...
**Activation:** IC + Planning Chief + 1-4 `icc-ops-specialist` + Safety
Officer.
...
- Introducing a new architectural pattern (a new plugin category, a new
  cross-cutting concern like the ICC gate hook itself)
...
execution instead of Agent-tool fan-out (offered by `/icc-execute`) when
