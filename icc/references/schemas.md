...
Returned by `icc-situation-analyst`.
...
Returned by `icc-planning-chief` (and, for deploy/env/migration concerns
only, `icc-logistics-chief` — see its own return shape below).
...
Returned by `icc-ops-specialist`.
...
Returned by `icc-safety-officer`.
...
Returned by `icc-commander`, one decision per invocation, at the four
...
| `type` / `verdict` / `disposition` | enum per point | The decision itself — see `agents/icc-commander.md` for each point's enum |
