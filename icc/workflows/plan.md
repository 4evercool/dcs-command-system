...
hash-stamped approval marker that opens the gate for /icc-execute.
</purpose>
...
there's no open incident — run `/icc-new` first. If `phase` is
...
approval once IAP.md changes) or actually meant `/icc-status` /
`/icc-execute`.
...
`$HOME/.claude/icc/templates/202-OBJECTIVES.md`. If this is a re-plan after
...
Spawn `icc-planning-chief` via Task, passing **only** the full text of
...
`icc-logistics-chief` in the same message with the same two files' text
...
(`command: iap_review <verdict> (IC=icc-commander)`). **If this session is
...
visibly overlap anyway: **reject.** Re-spawn `icc-planning-chief` with a
...
- `203-ORG.md` — activated positions this period, from
  `$HOME/.claude/icc/templates/203-ORG.md`.
- `204-TASKING/S1.md`, `S2.md`, ... — one per tasking, transcribed from the
  chief's `taskings[]`, using `$HOME/.claude/icc/templates/204-TASKING.md`.
- `IAP.md` — integrated from `$HOME/.claude/icc/templates/IAP.md`: links
...
Use `AskUserQuestion` — **not** plan mode. This is deliberate: ICC's own
...
- **Reject:** the incident doesn't proceed this period — return to intake
  framing (more analyst work via a fresh `/icc-new`-style investigation)
...
point 1, from `/icc-new`) and a `command: iap_review` entry (command point
...
the missed command point now** (spawn `icc-commander` if not Fable, decide
...
convenient in context — the value must match what `icc_gate.py` computes,
...
Update `<project>/.icc/ACTIVE` to `<slug>|<type>|execution`.
...
Tell the Owner the gate is open and the next step is `/icc-execute`.
