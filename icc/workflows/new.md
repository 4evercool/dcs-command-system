...
Gathers intel via icc-situation-analyst subagents, writes 201-BRIEF.md,
...
@$HOME/.claude/icc/references/typing.md
...
Determine the project root. Verify `<project>/.icc/` exists — if not, stop
and tell the Owner to run `/icc-init` first.
...
tell the Owner to finish it (`/icc-plan`, `/icc-execute`, or `/icc-close`
as appropriate — see `/icc-status` for exactly which) or explicitly close
...
Spawn 1-2 `icc-situation-analyst` subagents via the Task tool (in parallel,
...
Use `$HOME/.claude/icc/templates/201-BRIEF.md`. Fill in Symptom, Evidence
...
214-LOG.md as `command: typed <N> (IC=icc-commander)`. **If this session
...
2. Spawn **one** `icc-ops-specialist` via Task with that inline tasking —
...
4. Report a one-line AAR-equivalent to the Owner in chat: what was
   ...
   typing table.
5. Done. No `/icc-plan`, no `/icc-execute`, no `/icc-close` — the
   incident never opened a gate to begin with.

## 7b. Type 3 / Type 1 — open a gated incident

1. Create `<project>/.icc/incidents/<YYYY-MM-DD>-<slug>/` (slug: a short
...
3. Initialize `214-LOG.md` from the template, with the incident's slug and
   ...
   (`command: typed {N} -- <rationale> (IC=<icc-commander|this Fable
   session>)`) — `/icc-plan`'s pre-stamp checklist will refuse to stamp an
   approval if this entry is missing.
4. Write `<project>/.icc/ACTIVE` with content: `<slug>|<type>|planning`
   (exact pipe-delimited format — the gate hook parses this literally).
5. Tell the Owner the incident is open and the next step is `/icc-plan`.
